...
export const getUserRecommendations = async (req: Request, res: Response) => {
    ...
    try {
        ...
        try {
            ...
            const newsIds = recommendations.map((r: any) => {
                // Handle cases where news_id might be "db_1" or just 1
                const idStr = String(r.news_id);
                return idStr.startsWith('db_') ? parseInt(idStr.replace('db_', '')) : parseInt(idStr);
            ...
            // Combine article data with scores and SORT STRICTLY by score
            const result = newsArticles.map(article => {
                const rec = recommendations.find((r: any) => {
                    const rId = String(r.news_id).startsWith('db_')
                        ? parseInt(String(r.news_id).replace('db_', ''))
                        : parseInt(String(r.news_id));
                    return rId === article.id;
                });
                return {
                    ...article,
            ...
            // 2. Fallback: return latest news with random scores for UI demonstration if ML service is down
